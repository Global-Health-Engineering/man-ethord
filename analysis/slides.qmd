...
application_budget_long <- application_budget |>
  ...
  mutate(
    main_category = case_when(
      ...
      str_starts(budget_item, "other_") ~ "other",
...
application_budget_long |> 
  ...
  ggplot(aes(x = fct_reorder(main_category, total), y = total, fill = sub_category)) +
  geom_col() +
  geom_text(aes(label = scales::comma(total, prefix = "CHF ", accuracy = 1)),
            hjust = -0.1,
            size = 3.5) +
  coord_flip() +
  labs(
    title = "Budget Distribution by Main Category",
    subtitle = "Total budget allocation across all funded projects",
    x = NULL,
    y = "Total Budget (CHF)"
  ) +
